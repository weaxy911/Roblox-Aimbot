...
local MainFrame = Library:CreateWindow({
	Name = "WeaxyHUB Project",
	Themeable = {
		Image = "97365143429873",
		Info = "Made By Weaxy",
		Credit = false
	},
	Background = "",
...
FunctionsSection:AddButton({
	Name = "Copy Discord",
	Callback = function()
		setclipboard("https://discord.gg/rocondo")
	end
})

local UserInputService = game:GetService("UserInputService")
local ToggleKey = Enum.KeyCode.RightControl -- Varsayılan tuş

-- Tuş dinleyici
UserInputService.InputBegan:Connect(function(input, gpe)
	if not gpe and input.KeyCode == ToggleKey then
		Library:Toggle() -- Menü aç/kapa
	end
end)
...
-- Tuş dinleyici (Unload için)
UserInputService.InputBegan:Connect(function(input, gpe)
	if not gpe and input.KeyCode == ToggleKey then
		Library.Unload() -- Menü tamamen kapanır
	end
end)
...
do
    ...
    Aux.OnInvoke = function(Answer)
        ...
        -- "Yes" seçilirse Discord linkini panoya kopyala
        setclipboard("https://discord.gg/rocondo") 
        game.StarterGui:SetCore("SendNotification", {
            Title = "🎆  WeaxyHUB  🎆",
            Text = "Discord sunucu linki panoya kopyalandı!",
            Duration = 5,
            Icon = "rbxassetid://97365143429873"
        })
    end

    game.StarterGui:SetCore("SendNotification", {
        Title = "🎆  WeaxyHUB Discord 🎆",
